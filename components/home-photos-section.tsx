            ...
            >
              {TRUST_ITEMS.map(({ key, text, Icon }) => {
                ...
                return (
                  ...
                  >
                    {isGoogle ? (
                      <div className="flex w-full flex-col gap-1 sm:items-center sm:gap-1 sm:text-center">
                        {/* Logo + estrellas arriba del texto (mismo X, Y simétrico) */}
                        <div className="flex items-center gap-3 sm:flex-col sm:gap-2.5">
                          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10">
                            <GoogleGIcon className="h-4 w-4" />
                          </div>
                          <div className="flex min-w-0 flex-1 flex-col gap-1 sm:flex-none sm:items-center">
                            <div className="flex justify-center">
                              <FiveStars className="-translate-x-5 sm:translate-x-0" starClassName="h-4 w-4" />
                            </div>
                            <p className="text-[13px] font-semibold leading-snug text-foreground/80 sm:min-h-[2.5rem] sm:text-xs">
                              {text}
                            </p>
                          </div>
                        </div>
